...
(defun at-least-one-color (n c k)
  (cond ((> c k) nil) ;if exceed the bound, return nil
    (t (cons (node2var n c k) (at-least-one-color n (+ c 1) k))) ;assign this node with color c and continue with color c+1
  )
  )

(defun compare-pairs (n c k cur)
  (cond ((>= c k) nil) ;if this c is the last one, stop
    ;examine the pair (cur c), and they cannot both be true, and continue comparing (cur c+1)
    (t (cons (list (* -1 (node2var n cur k)) (* -1 (node2var n (+ 1 c) k))) (compare-pairs n (+ 1 c) k cur)))
  )
)
...
(defun at-most-one-color (n c k)
  (cond ((> c k) nil) ;if exceed the bound, return nil
    ;examine pairs (cur c) with cur start from c to k
    (t (append (compare-pairs n c k c) (at-most-one-color n (+ 1 c) k)))
  )
  )
...
(defun not-equal (l1 l2)
  ; if either l1 or l2 is empty, stop
  (cond ((null l1) nil)
    ((null l2) nil)
    ; n1 and n2 cannot be asigned with the same color (cannot both have positive value in the same position)
    (t (cons (list (* -1 (car l1)) (* -1 (car l2))) (not-equal (cdr l1) (cdr l2))))
  )
)
...
(defun generate-edge-clauses (e k)
  (let ((l1 (at-least-one-color (first e) 1 k)) (l2 (at-least-one-color (second e) 1 k)))
    (not-equal l1 l2)
  )
)
